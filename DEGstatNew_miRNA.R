DEGstatNew_miRNA<-function(datamatrix,pval_cutoff,upperfoldchange_cutoff,lowerfoldchange_cutoff,pval_adj_used,ctl.subtype,exp.subtype,control.size,tissue)
     {
         ...
         # nrow(RNA_Data)#19685
          ...
          print("The total no of significant miRNA:");print(n1);#582
          print("The total no of Upregulated miRNA:"); print(n2);#260
          print("The total no of Downregulated miRNA:");print(n3);#322
          fURGlist_srt_cntlvsrest_r$X<-rownames(fURGlist_srt_cntlvsrest_r)
          fDRGlist_srt_cntlvsrest_r$X<-rownames(fDRGlist_srt_cntlvsrest_r)
          fDEGlist_pvalFCfilt_cntlvsrest_r$X<-rownames(fDEGlist_pvalFCfilt_cntlvsrest_r)
          a<-list(UG_Name= fURGlist_srt_cntlvsrest_r$X,DG_Name=fDRGlist_srt_cntlvsrest_r$X)
          print("the Upregulated miRNA Names are as follows....")
          print(a$UG_Name)
          print("the Downregulated miRNA Names are as follows....")
          print(a$DG_Name)
          
          write.table(fURGlist_srt_cntlvsrest_r, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".URmiRNA.csv",sep=""),sep=",",col.names = TRUE,row.names = FALSE)
          #write.table(fURGlist_srt_cntlvsrest_r, paste("UG_ADENOvs",subtype, ".csv",sep=""), sep=",", row.names=FALSE, col.names=TRUE,quote=FALSE)
          write.table(fDRGlist_srt_cntlvsrest_r, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".DRmiRNA.csv",sep=""),sep=",",col.names = TRUE,row.names = FALSE)
          #write.table(fDRGlist_srt_cntlvsrest_r,paste("DG_ADENOvs",subtype, ".csv",sep=""), sep=",", row.names=FALSE, col.names=TRUE,quote=FALSE)
          write.table(fDEGlist_pvalFCfilt_cntlvsrest_r, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".DEmiRNA.csv",sep=""),sep=",",col.names = TRUE,row.names = FALSE)
          ...
          heatmap_matrix<-heatmap1[,-1]
          write.table(heatmap_matrix, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".DEmiRNA.dataval.csv",sep=""),sep=",",col.names = FALSE,row.names = TRUE)
          write.table(heatmap_matrix, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".input_WGCNA_dataval.csv",sep=""),sep=",",col.names = FALSE,row.names = FALSE)
          gene_list<-rownames(heatmap1)
          a<-as.data.frame(gene_list)
          gene_number<-rownames(a)
          write.table(gene_list, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".DEmiRNA.GeneNames.csv",sep=""),sep=",",col.names = FALSE,row.names = FALSE)
          write.table(gene_number, file=paste0(tissue,".",ctl.subtype,".vs.",exp.subtype,".DEmiRNA.GeneNumbers.csv",sep=""),sep=",",col.names = FALSE,row.names = FALSE)
